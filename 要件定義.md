...
* 6VMすべて **Debian 12** を採用する。GitLab Linuxパッケージのサポート対象である。([GitLab ドキュメント][1])
...
本システムは、学習目的（実務寄りの構成理解）として、GitLabを以下の **6VM** に分割して構築する。
...
* GitHub Actionsのself-hosted runnerとして稼働する。
* GitHub → GitLabのミラーpushを実行する。
...
* 6VMはすべて **固定IP** とする（具体的な割当値は実装側で設定する）。
...
* GitHub側の自動化（GitHub Actionsのself-hosted runner）により、GitHub → GitLab へミラーpushを実行して同期する。
* 認証方式は **SSH public key** を用いる。

  * GitLab側に **専用アカウント（機械ユーザー）** を作成し、当該アカウントへSSH公開鍵を登録する。([GitLab ドキュメント][9])
  * GitHub側に、対応するSSH秘密鍵をGitHub ActionsのSecretsとして登録して利用する。([GitHub ドキュメント][4])
  * 同期先のGitLabプロジェクトに対し、専用アカウントが書き込み可能な権限を保持する（全リポジトリに適用する）。
* 運用ルール：
...
* 現時点では **GitLab Runner（GitLab CI用）は導入しない**。
...
[9]: https://docs.gitlab.com/user/ssh/ "Use SSH keys with GitLab"
